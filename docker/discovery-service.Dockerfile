...
# every module pom must be visible for `-pl discovery-service -am` to resolve the reactor.
...
RUN --mount=type=cache,target=/root/.m2 \
    ./mvnw -B -pl discovery-service -am package -DskipTests
