...
# every module pom must be visible for `-pl gateway-service -am` to resolve the reactor.
...
RUN --mount=type=cache,target=/root/.m2 \
    ./mvnw -B -pl gateway-service -am package -DskipTests
...
COPY --from=build /src/gateway-service/target/*.jar app.jar
